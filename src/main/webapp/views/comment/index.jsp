...
layui.config({
	...
	table.render({
		elem: '#ZX_table_list',
		 height: 'full-100', 
		url: '${ctxStatic}/sys/comment/getList',
		cols: [[
			{checkbox: true, fixed:true},
			{field:'id', title:'ID', align:'center', width:70, sort:true},
			{field:'userid', title:'发表人ID', align:'center',width:100},
			// {field:'touserid', title:'接收人ID', align:'center', width:100, sort:false},
			{field:'tovideoid', title:'视频对象', align:'center', width:100, sort:false},
			{field:'username', title:'发表人昵称', align:'center',width:100},
			{field:'comment', title:'内容', align:'center', width:400, sort:false},
	...
	$(".btnAdd").click(function(){
		...
		layer.open({
			type: 2,
			title: '添加评论',
			content: '${ctxStatic}/sys/comment/goAdd',
			area: ['650px', '500px'],
			btnAlign: 'center',
			btn: ['确定', '取消'],
			yes: function(index, layero){
				...
				iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
					$.ajax({
						type: "POST",
						url: '${ctxStatic}/sys/comment/insert',
						data: data.field,
						dataType: "json",
	...
	$(".allDel").click(function(){	
		...
		layui.layer.confirm('您真的删除么', function(index){
			...
			$.ajax({
				type: "GET",
				url: '${ctxStatic}/sys/comment/delete?id='+ids.toString(),
				dataType: "json",
				success: function(data){
					layer.msg(data.msg);
					if(data.code==0){
	...
	table.on('tool(ZX_table_list)', function(obj){
		...
		if(layEvent === 'del'){
			...
			layer.confirm('确定删除？', function(index){
				$.ajax({
					type: "GET",
					url: '${ctxStatic}/sys/comment/delete?id='+obj.data.id,
					dataType: "json",
					success: function(data){
						if(data.code==0){
							layer.msg(data.msg);
			...
			layer.open({
				type: 2,
				title: '修改评论',
				content: '${ctxStatic}/sys/comment/goEdit?id='+obj.data.id,
				area: ['650px', '500px'],
				btnAlign: 'c',
				btn: ['确定', '取消'],
				yes: function(index, layero){
					...
					iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
						$.ajax({
							type: "POST",
							url: '${ctxStatic}/sys/comment/update',
							data: data.field,
							dataType: "json",
							success: function(data){
								if(data.code==0){
