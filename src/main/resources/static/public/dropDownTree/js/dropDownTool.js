...
(function ($) {
    ...
    function checkNode(event, treeId, treeNode) {
        ...
        //多选
            ...
            for (var i = 0; i < nodes.length; i++) {
            	//部门id舍去
            	if(nodes[i].id.indexOf('branch')==-1)
            	{
            ...
            //获取前台页面的顺序
            var idIndex = $("#" + treeId).parents('[cytype="dropDownTool"]').find(".form-input").attr("idIndex");
            //给隐藏框赋值
            $('#userIds'+idIndex).val(checkIds);
    ...
    function createDropDownDom($inputValue, cyProps) {
        ...
        var _indexId= $($inputValue).attr("idIndex") || "";
        ...
        var inputHtml =
            $inputValue.html([
                '<input type="hidden" class="form-input" idIndex="'+_indexId+'" name="' + _name + '" value="' + _value + '">',
                '<textarea rows="3" cols="20" style="resize:none; height:60px;width:500px;font-size:20;" type="text"  readonly="true" placeholder="' + _tips + '" id="' + txtId + '" class="combo-text layui-input" >',
                '</textarea>',
            ].join(""));
    ...
    function BindPanelShowHide(obj) {
        ...
        if ($treeDiv.is(":hidden")) {
            $treeDiv.show();
            $treeDiv.css("z-index", -1); //修改弹出层优先级，低于对话框
    ...
    function getTreeData(url) {
    	var data="";
    	//判断jsp页面是否有存在的parentExistJson 不用url去查询
    	if(parentExistJson!=undefined)
    	{
    		data = parentExistJson;
    	}
    	else
    	{
    		$.ajax({
    ...
    function setFontCss_ztree(treeId, treeNode) {
        var treeObj = $.fn.zTree.getZTreeObj(treeId);
        //修改部门显示红色  人员显示黑色
        if(treeNode.id && treeNode.id.indexOf('branch')==-1)
        {
       ...
       /* if (treeNode[treeObj.key_code] == 0) {
            ...
            //父节点
            return (!!treeNode.highlight) ? {color:"#ffff00", "font-weight":"bold"} : {color:"#ff00ff", "font-weight":"bold"};
        }*/
    }
